...
class Trace(ttk.Frame):
    def __init__(self, container):
        ...
        self.columnconfigure(0, weight=3)
        self.fig_trace, self.ax_trace = plt.subplots(len(container.rois), 1, figsize=(10, 5))
        self.colors = container.colors
        if len(container.rois) == 1:
            self.ax_trace = [self.ax_trace]
        self.trace_points = [deque(maxlen=1000) for _ in range(len(container.rois))]
        self.lines = [ax.plot([], [], color=self.colors[i % len(self.colors)])[0] for i, ax in enumerate(self.ax_trace)]
        self.__create_widgets()

    def update_trace(self):
        idx = 0
        for ax, trace in zip(self.ax_trace, self.trace_points):
            ax.clear()  # Clear the existing plot
            ax.plot(range(len(trace)), list(trace), color=self.colors[idx])
            idx += 1 
        self.fig_trace.canvas.draw()  # Refresh the canvas
...
class Image(ttk.Frame):
    def __init__(self, container):
        ...
        self.rectangles = container.rectangles
        self.fig_img, self.ax_img = plt.subplots()
        self.__create_widgets()

    def update_image(self, image):
        self.ax_img.imshow(image)
        for (x1, x2, y1, y2, color) in self.rectangles:
            self.ax_img.add_patch(plt.Rectangle((x1, y1), x2 - x1, y2 - y1, fill=False, edgecolor=color, linewidth=2))
...
class Segment(ttk.Frame):
    ...
    def on_button_click(self, event, roi):
        ...
        self.rectangles.append((x1, x2, y1, y2, color))
...
class Next(ttk.Frame):
    def __init__(self, master, rois, rectangles, colors):
    ...
    def new_window(self):
        self.NewWindow = VisualisationWindow(self.rois, self.rectangles, self.colors)
        self.master.destroy()
...
class SegmentationWindow(tk.Toplevel):
    ...
    def __create_widgets(self):
        ...
        loading_frame = Next(self, segment_frame.selections, segment_frame.rectangles, segment_frame.colors)
        loading_frame.grid(column=0, row=1)

class VisualisationWindow(tk.Toplevel):
    def __init__(self, selections, overlay, colors):
        super().__init__()
    ...
    def acquisition(self):
        try:
            with Acquisition(directory=r'C:\Users\aysez\Documents\optical oscilloscope\debug', name="zelux_acq", image_process_fn=self.image_process_fn) as acq:
                events = multi_d_acquisition_events(num_time_points=500, time_interval_s=1)
                acq.acquire(events)
    ...
    def image_process_fn(self, image, metadata):
        ...
        try:
            ...
            if datapoints is not None:
                if isinstance(datapoints, np.float64):
                    self.trace_frame.trace_points[0].append(datapoints)
                else:
                    for idx, (trace, data) in enumerate(zip(self.trace_frame.trace_points, datapoints)):
                        # Debug prints to trace the data appending process
                        print(f"Trace {idx} before appending: {trace}")
                        print(f"Data to append: {data}")
                        
                        trace.append(data)
                        
                        print(f"Trace {idx} after appending: {trace}")
                        print(f"Appending data: {data} to trace {idx}")
                        
                        # Update the corresponding plot line data
                        self.trace_frame.lines[idx].set_data(range(len(trace)), trace)
                self.trace_frame.update_trace()
                self.trace_frame.fig_trace.canvas.draw()
                self.trace_frame.fig_trace.canvas.flush_events()
        except Exception as e:
            print(f'Datapoints: {datapoints}, Exception: {e}')
        finally:
    ...
    def handle_frame(self, image): # selections a LIST of rois
        ...
        def extract_rois(roi):
            (x1, x2, y1, y2) = roi
            pixels = np.mean(image[x1:x2, y1:y2])
            return pixels
        # elif len(self.rois) > len(self.batch[0][1]):
        #    self.batch.extend([(batch_idx, []) for _ in range(len(self.rois) - len(self.batch[0][1]))])
        if len(self.rois) == 1:
            pixels = extract_rois(self.rois[0])
            self.batch.append((batch_idx, pixels))
        else:
            new_pixels = []
            for roi in self.rois:
                pixels = extract_rois(roi)
                new_pixels.append(pixels)
            self.batch.append((batch_idx, new_pixels))  # Append pixel values to the existing list for this ROI
        return self.frame_idx

    def process_frame(self):
        idx_list, rois = zip(*self.batch)
        last_idx = idx_list[-1]
        transposed = list(zip(*rois[1:]))
        roi_list = [list(x) for x in transposed]
        processed_points = []
        if isinstance(rois[-1], list):
            for roi in roi_list:
                if last_idx > 0 and last_idx % 2 == 0:
                    pixels = np.array(roi)
                    blue = pixels[:last_idx-1:2].astype(float)
                    green = pixels[1:last_idx:2].astype(float)
                    blue /= ndimage.uniform_filter(blue, size=1000, mode='nearest', origin=self.origin)
                    green /= ndimage.uniform_filter(green, size=1000, mode='nearest', origin=self.origin)
                    proc_points = blue / green
                    processed_points.append(proc_points[-1])
            return processed_points
        else:
            try:
                pixels = np.array(rois[1:])
                blue = pixels[:last_idx-1:2].astype(float)
                green = pixels[1:last_idx:2].astype(float)
                blue /= ndimage.uniform_filter(blue, size=1000, mode='nearest', origin=self.origin)
                green /= ndimage.uniform_filter(green, size=1000, mode='nearest', origin=self.origin)
                ...
                print(f"Error processing ROI single ROI: {e}")
